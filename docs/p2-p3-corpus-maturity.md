...
| --- | ---: | --- |
| `genealogicalRelations` | 26 | Derived or secondary assertions; the Hoysala, Wadiyar, Western Ganga and Kadamba (Mayurasharma-Kakusthavarma, inscription-attested via the Talagunda pillar) long bridges are now split into individual-generation links (still needs-review), plus the Vengi and Tuluva-line proof packets — none are final family-tree proof |
| `coinRecords` | 10 | 8 of 10 leads now have located catalogue/metal/weight gates from specialist numismatic catalogues (Numista, CoinIndia) or, for Maski, the general Mauryan karshapana type with an honest note that no Maski-specific hoard/specimen has been matched. The Hoysala Garuda/fanam record's name was also corrected: the catalogued Hoysala fanam shows a lion (sardula) obverse and boar reverse, not Garuda. The 2 remaining leads (Western Ganga and Krishnadevaraya/Achyutaraya Parampare image-plate leads) are explicitly visual-plate leads, not catalogue sources, and are untouched by this pass. Image and findspot gates stay open across all 10 |
| `manuscriptWitnesses` | 14 | 7 witnesses now have real Internet Archive editions (Kumaravyasa Bharata, Torave Ramayana, Kavirajamarga, Vikramarjuna Vijaya, and new Ajitapurana/Shantipurana witnesses) with `editionComparison` and `access` located; shelfmark and independent-review gates stay open; two dead CESCK catalogue links (Vaddaradhane, Adipurana) were caught and downgraded rather than left pointing nowhere |
| `boundaryEvidence` | 9 | Schematic evidence packets, including new Chalukya inscription-cluster and Keladi coastal-interior administrative packets; 7 of 9 packets now have sourced inscription, campaign, treaty or administrative-unit evidence for some gates (Hoysala x2, Tipu, Vijayanagara/Krishnadevaraya, Rashtrakuta x2, Western Ganga); each still blocked on at least a reign/period-specific source map. The Chalukya/Aihole cluster and the Keladi admin-zone packets remain untouched by this pass; not research-grade boundary corpus |
