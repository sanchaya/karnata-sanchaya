...
Atlas v0.28.3 expands the same model into a full palaeographic classification. `scriptEvolution` records now carry broad `morphologicalStage`, `dynasticClassification`, `evidenceMedium`, `visualTraits` and bilingual `morphologyNote` fields. The public `#scripts` explorer exposes these fields while keeping every phase review-gated. The current ladder covers Ashokan/Satavahana Brahmi root evidence, Kadamba/Proto-Kannada, Badami Chalukya, Rashtrakuta, Kalyani Chalukya-Hoysala, Vijayanagara-Nayaka/Telugu-Kannada divergence and Mysore/modern print Kannada. New Maski and Brahmagiri inscription packets, Satavahana context, coin, genealogy, samanta, boundary, district-history and manuscript witnesses are routing records for review, not final palaeographic or political conclusions.
