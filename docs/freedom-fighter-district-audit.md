...
The 20 `karnataka-event-connection` records also retain provisional activity years and controlled action leads—imprisonment/detention, arrest/capture, death/martyrdom, movement/resistance or armed action. The dedicated public `#freedom` explorer combines these records in a district-filtered map, list and keyboard-accessible timeline, with direct links to the person packet, main atlas and moderated contribution workflow. People Explorer links to this page, while the main atlas projects the same inputs into its **Freedom movement / ಸ್ವಾತಂತ್ರ್ಯ ಚಳವಳಿ** timeline category and map layer. These are dashed, district/city-centre `needs-review` projections linked back to their candidate and printed-page locator; the ranges describe the cited historical connection, not the person’s lifespan.

Curated profiles can additionally retain `placeAssociations` for independently named villages, towns and event sites. Each association has its own relationship kind and citation. Never convert a district-only lead into a village claim or present a district-centre point as the person’s exact location.
