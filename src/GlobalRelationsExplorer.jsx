...
const layoutGroupEdges=edges=>{
  const nodeMap=new Map()
  edges.forEach(edge=>{nodeMap.set(edge.from.id,edge.from);nodeMap.set(edge.to.id,edge.to)})
  const ids=[...nodeMap.keys()]
  const level=new Map(ids.map(id=>[id,0]))
  for(let pass=0;pass<ids.length;pass++)edges.forEach(edge=>{const next=level.get(edge.from.id)+1;if(next>level.get(edge.to.id))level.set(edge.to.id,next)})
  const maxLevel=Math.max(0,...level.values())
  const byLevel=new Map()
  ids.forEach(id=>{const lvl=level.get(id);byLevel.set(lvl,[...(byLevel.get(lvl)||[]),id])})
  const rowGap=15
  const maxRows=Math.max(1,...[...byLevel.values()].map(list=>list.length))
  const height=Math.max(56,(maxRows-1)*rowGap+32)
  const width=maxLevel<=4?100:16+maxLevel*22
  const positions={}
  byLevel.forEach((list,lvl)=>{
    const x=8+lvl*((width-16)/Math.max(maxLevel,1))
    list.forEach((id,index)=>{positions[id]={x,y:height/2+(index-(list.length-1)/2)*rowGap,level:lvl}})
  })
  return {nodes:[...nodeMap.values()],positions,height,maxLevel,width}
}
...
function GenealogyNetwork({locale,t}){
  ...
  return <details className="relations-network-panel" open={open} onToggle={event=>setOpen(event.currentTarget.open)}><summary><span><b>{t.networkTitle}</b><small>{t.networkIntro}</small></span></summary><div className="relations-network-toolbar"><label><span>{t.networkKind}</span><select value={kind} onChange={event=>setKind(event.target.value)}><option value="all">{t.allNetworks}</option><option value="genealogy">{t.genealogy}</option><option value="feudatory">{t.feudatory}</option></select></label><div className="relations-network-legend"><span><i className="source-backed"></i>{t.sourceBacked}</span><span><i className="derived"></i>{t.derivedEdge}</span></div></div><div className="relations-network-groups">{[...grouped.values()].map(group=>{const {nodes,positions,height,maxLevel,width}=layoutGroupEdges(group.edges);const label=text(group.polity?.name,locale)||t.all;const wide=maxLevel>4;return <article key={group.polity?.id||'other'} className={wide?'wide':''}><strong>{label}</strong><div className={wide?'relations-network-mini-scroll':''}><svg className="relations-network-mini" viewBox={`0 0 ${width} ${height}`} style={wide?{width:width*6,maxWidth:'none'}:undefined} role="img" aria-label={label}>{group.edges.map(edge=>{const from=positions[edge.from.id],to=positions[edge.to.id];return <line key={edge.id} className={`${edge.kind} ${edge.evidenceLevel==='derived'?'derived':'source-backed'}`} x1={from.x} y1={from.y} x2={to.x} y2={to.y}/>})}{nodes.map(node=>{const point=positions[node.id],local=localPolityIds.has(node.polityId||node.id);const edgeCol=point.level===0?'start':point.level===maxLevel?'end':'middle';const labelX=edgeCol==='start'?point.x+4.5:edgeCol==='end'?point.x-4.5:point.x;const labelY=edgeCol==='middle'?(point.level%2===0?point.y-5:point.y+7.5):point.y+1.2;return <g key={node.id} className={local?'local':'external'}><circle cx={point.x} cy={point.y} r="3.2"/><text x={labelX} y={labelY} textAnchor={edgeCol}>{text(node.name,locale)}</text></g>})}</svg></div>{group.edges.map(edge=>{const source=edge.record.citations?.[0]&&sourceById.get(edge.record.citations[0].sourceId);return <div className={edge.evidenceLevel==='derived'?'derived':'source-backed'} key={edge.id}><span>{text(edge.from.name,locale)}</span><i aria-hidden="true">→</i><span>{text(edge.to.name,locale)}</span><small>{edge.kind==='genealogy'?t.genealogy:t.feudatory} · {t.evidenceLevel}: {edge.evidenceLevel} · {edge.reviewStatus}</small>{source&&<em>{text(source.title,locale)}</em>}</div>})}</article>})}</div></details>
}
...
export default function GlobalRelationsExplorer({locale='kn',mapTheme='modern',setMapTheme,onOpenAtlas}){
  ...
  const selectMatrix=(nextCorridor,nextCategory)=>{setFocus('all');setRegion('all');setCorridor(nextCorridor);setCategory(nextCategory);setControl('all');setPolity('all');setCentury('all');setQuery('');document.querySelector('.relations-workspace')?.scrollIntoView({behavior:'smooth',block:'start'})}
