...
function RelationsMatrix({locale,t,onSelect,canExport=false}){
  ...
  const exportMatrix=()=>{const payload={schemaVersion:atlasData.meta.schemaVersion,exportedAt:new Date().toISOString(),policy:'Zero cells are research gaps, not proof that no historical connection existed. Needs-review records remain provisional.',corridors:matrix.map(row=>({...row,label:t.corridors[row.corridor]})),records:records.map(item=>({id:item.id,corridor:item.corridor,category:item.category,reviewStatus:item.reviewStatus,date:item.date,countries:item.countries}))};const url=URL.createObjectURL(new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}));const link=document.createElement('a');link.href=url;link.download='karnataka-atlas-relations-corridor-matrix.json';link.click();URL.revokeObjectURL(url)}
  return <details className="relations-matrix" open><summary><span><b>{t.matrixTitle}</b><small>{t.matrixIntro}</small></span></summary><div className="matrix-scroll"><table><thead><tr><th>{t.corridor}</th>{matrixCategories.map(category=><th key={category}>{t[category]}</th>)}</tr></thead><tbody>{matrix.map(row=><tr key={row.corridor}><th>{t.corridors[row.corridor]}</th>{matrixCategories.map(category=>{const count=row.cells[category];return <td key={category}><button className={count?'covered':'gap'} title={count?`${count} ${t.results}`:t.coverageGap} onClick={()=>onSelect(row.corridor,category)}><b>{count}</b><span>{count?t.results:t.coverageGap}</span></button></td>})}</tr>)}</tbody></table></div>{canExport?<button className="matrix-export" onClick={exportMatrix}>{t.exportMatrix}</button>:<span className="export-restricted-note">{exportCopy[locale]}</span>}</details>
}
...
export default function GlobalRelationsExplorer({locale='kn',mapTheme='modern',setMapTheme,onOpenAtlas,canExport=false}){
  const t=copy[locale]
  ...
  const filtered=useMemo(()=>records.filter(item=>{
    ...
    return (!needle||item.searchText.includes(needle)||item.countries.join(' ').toLowerCase().includes(needle))&&(region==='all'||item.region===region)&&(corridor==='all'||item.corridor===corridor)&&(category==='all'||item.category===category)&&(control==='all'||item.controlKind===control)&&(polity==='all'||item.polityIds.includes(polity))&&(century==='all'||itemCentury===Number(century))
  }),[query,region,corridor,category,control,polity,century])
