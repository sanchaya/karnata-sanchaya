...
function GenealogyNetwork({locale,t}){
  ...
  const nodes=[...new Map(visible.flatMap(edge=>[edge.from,edge.to]).map(node=>[node.id,node])).values()]
  const nodePositions=Object.fromEntries(nodes.map((node,index)=>{const angle=(Math.PI*2*index)/Math.max(nodes.length,1)-Math.PI/2;return [node.id,{x:50+38*Math.cos(angle),y:50+34*Math.sin(angle)}]}))
  return <section className="relations-network-panel"><header><div><h3>{t.networkTitle}</h3><p>{t.networkIntro}</p></div><label><span>{t.networkKind}</span><select value={kind} onChange={event=>setKind(event.target.value)}><option value="all">{t.allNetworks}</option><option value="genealogy">{t.genealogy}</option><option value="feudatory">{t.feudatory}</option></select></label></header><div className="relations-network-graph" aria-label={t.networkTitle}><svg viewBox="0 0 100 100" role="img">{visible.map(edge=>{const from=nodePositions[edge.from.id],to=nodePositions[edge.to.id];return <line key={edge.id} className={`${edge.kind} ${edge.evidenceLevel==='derived'?'derived':'source-backed'}`} x1={from.x} y1={from.y} x2={to.x} y2={to.y}/>})}{nodes.map(node=>{const point=nodePositions[node.id],local=localPolityIds.has(node.polityId||node.id);return <g key={node.id} className={local?'local':'external'}><circle cx={point.x} cy={point.y} r="4.2"/><text x={point.x} y={point.y-6}>{text(node.name,locale)}</text></g>})}</svg><div><span><i className="source-backed"></i>{t.sourceBacked}</span><span><i className="derived"></i>{t.derivedEdge}</span></div></div><div>{[...grouped.values()].map(group=><article key={group.polity?.id||'other'}><strong>{text(group.polity?.name,locale)||t.all}</strong>{group.edges.map(edge=>{const source=edge.record.citations?.[0]&&sourceById.get(edge.record.citations[0].sourceId);return <div className={edge.evidenceLevel==='derived'?'derived':'source-backed'} key={edge.id}><span>{text(edge.from.name,locale)}</span><i aria-hidden="true">→</i><span>{text(edge.to.name,locale)}</span><small>{edge.kind==='genealogy'?t.genealogy:t.feudatory} · {t.evidenceLevel}: {edge.evidenceLevel} · {edge.reviewStatus}</small>{source&&<em>{text(source.title,locale)}</em>}</div>})}</article>)}</div></section>
}
