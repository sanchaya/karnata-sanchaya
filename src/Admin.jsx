...
const collectSourceUses = (value, sourceId, path='record', output=[]) => {
  if (Array.isArray(value)) value.forEach((item,index)=>collectSourceUses(item,sourceId,`${path}[${index}]`,output))
    ...
    if (key==='sourceId' && item===sourceId) output.push(path)
    else collectSourceUses(item,sourceId,`${path}.${key}`,output)
  })
  return output
}
  ...
  return <div className="admin-shell" lang={locale}>
    ...
    <main className="admin-main">
      ...
      <section className="record-editor">
        ...
        {collection==='sources'&&<section className="admin-source-usage" aria-labelledby="source-usage-title"><h3 id="source-usage-title">{t.sourceUsage}</h3><p>{t.sourceUsageNote}</p>{selectedSourceUses.length?<ul>{selectedSourceUses.map((use,index)=><li key={`${use.collection}-${use.id}-${index}`}><button type="button" onClick={()=>{const record=(data[use.collection]||[]).find(item=>item.id===use.id);if(record){selectCollection(use.collection);selectRecord(record)}}}>{t.collections[use.collection]||use.collection} · {use.title}</button><code>{use.locator}</code></li>)}</ul>:<small>{t.noSourceUsage}</small>}</section>}
