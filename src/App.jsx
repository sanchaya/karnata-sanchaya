...
function EventRail({locale,t,year,stories,selectedId,onSelect}){
  ...
  return <section className="event-rail"><div className="event-rail-title"><strong>{t.timelineStories}</strong><label><span>{t.searchTimeline}</span><input aria-label={t.searchTimeline} value={query} onChange={event=>setQuery(event.target.value)} placeholder={t.searchTimelinePlaceholder}/></label></div><div className="timeline-filters">{['all','wars','political','connections','people','literature','inscriptions','temples','monuments','reigns','territory','culture'].map(value=><button key={value} className={category===value?'active':''} onClick={()=>setCategory(value)}><span aria-hidden="true">{value==='all'?'':objectIcon(value==='temples'?'temple':value==='monuments'?'monument':value==='wars'?'war':value==='connections'?'connection':value==='inscriptions'?'inscription':value==='people'?'person':value==='reigns'?'reign':value==='territory'?'territory':value)}</span>{t.timelineCategories[value]}</button>)}</div><div className="event-track">{visibleStories.map(story=>{const kind=objectKindFor(story);return <button key={`${story.storyKind}-${story.id}`} className={`${story.storyKind} ${story.reach?'connection-event':''} ${selectedId===story.id?'selected':''} ${Math.abs(story.year-year)<=18?'near':''}`} onClick={()=>onSelect(story)} style={{'--event-color':colorFor(story)}}><i aria-hidden="true"><span>{objectIcon(kind)}</span></i><span>{story.date.precision==='circa'?(locale==='kn'?'ಸು. ':'c. '):''}{timelineYearLabel(story.year,locale)}</span><strong>{primary(story.name,locale)}</strong><em>{labelFor(story)}</em></button>})}{visibleStories.length===0&&<p className="timeline-empty">{t.noTimelineStories}</p>}</div></section>
}
    ...
    {view==='atlas'&&<main id="atlas">
      ...
      <section className={`map-stage theme-${mapTheme} ${compareYear!=null?'compare-mode':''}`}>
        ...
        <MapContainer center={[atlasMapView.lat,atlasMapView.lng]} zoom={atlasMapView.zoom} minZoom={3} scrollWheelZoom>
          ...
          {layers.boundaries&&active.filter(item=>item.type==='external-governance').map(phase=><Polygon key={phase.id} positions={phase.polygon} pathOptions={{color:phase.color,fillColor:phase.color,fillOpacity:(selected===phase.id ? .3 : .13),weight:selected===phase.id?3:2,dashArray:'8 6'}} eventHandlers={{click:()=>{setSelected(phase.id);setSelectedEvent(null);setSelectedTerritory(null);setSelectedCulture(null);clearRecordDetails()}}}></Polygon>)}
