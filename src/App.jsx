...
const publicDataDepth=(()=>{
  const historicalCollections=['events','people','works','inscriptions','culturalHeritage','reigns','territorialExtents','politicalRelations','externalGovernancePhases','districtHistoryResearch']
...
function EventRail({locale,t,year,stories,selectedId,onSelect,onCategoryChange}){
  ...
  const timelineCategories=['all','wars','political','connections','people','literature','inscriptions','temples','monuments','reigns','territory','culture']
  ...
  return <section className="event-rail"><div className="event-rail-title"><strong>{t.timelineStories}</strong><label><span>{t.searchTimeline}</span><input aria-label={t.searchTimeline} value={query} onChange={event=>setQuery(event.target.value)} placeholder={t.searchTimelinePlaceholder}/></label></div><select className="timeline-category-select" aria-label={t.timelineCategoryLabel} value={category} onChange={event=>chooseCategory(event.target.value)}>{timelineCategories.map(value=><option key={value} value={value}>{t.timelineCategories[value]}</option>)}</select><div className="timeline-filters">{timelineCategories.map(value=><button key={value} className={category===value?'active':''} onClick={()=>chooseCategory(value)}><span aria-hidden="true">{value==='all'?'':objectIcon(value==='temples'?'temple':value==='monuments'?'monument':value==='wars'?'war':value==='connections'?'connection':value==='inscriptions'?'inscription':value==='people'?'person':value==='reigns'?'reign':value==='territory'?'territory':value)}</span>{t.timelineCategories[value]}</button>)}</div><div className="event-track">{visibleStories.map(story=>{const kind=objectKindFor(story);return <button key={`${story.storyKind}-${story.id}`} data-story-category={story.storyCategory} className={`${story.storyKind} ${story.reach?'connection-event':''} ${selectedId===story.id?'selected':''} ${Math.abs(story.year-year)<=18?'near':''}`} onClick={()=>onSelect(story)} style={{'--event-color':colorFor(story)}}><i aria-hidden="true"><span>{objectIcon(kind)}</span></i><span>{story.date.precision==='circa'?(locale==='kn'?'ಸು. ':'c. '):''}{timelineYearLabel(story.year,locale)}</span><strong>{primary(story.name,locale)}</strong><em>{labelFor(story)}</em></button>})}{visibleStories.length===0&&<p className="timeline-empty">{t.noTimelineStories}</p>}</div></section>
}
...
export default function App(){
  ...
  const [showAllReviewCandidates,setShowAllReviewCandidates]=useState(true)
  ...
  const alreadyMappedReviewIds=new Set([...activeInscriptions,...activeMapCulture,...visibleHeritage,...nearbyEvents].map(item=>item.id))
  ...
  const coordinateTimelineCategory=category=>{const layerByCategory={wars:'events',political:'events',connections:'events',inscriptions:'inscriptions',temples:'temples',monuments:'culture',culture:'culture',reigns:'boundaries',territory:'territorialReach'};const layer=layerByCategory[category];if(layer)enableMapLayer(layer)}
  ...
  return <div className="app-shell" lang={locale}>
    ...
    <footer><span className="footer-brand"><img src={`${import.meta.env.BASE_URL}sanchaya-logo.png`} alt=""/>{atlasData.meta.title.kn} · Karnataka Historical Atlas · v{atlasData.meta.schemaVersion}</span><span className="footer-partners"><a href="#about"><span className="partner-icon"><img src={`${import.meta.env.BASE_URL}sanchaya-logo.png`} alt=""/></span>{locale==='kn'?'ನಮ್ಮ ಬಗ್ಗೆ':'About'}</a></span></footer>
  </div>
}
