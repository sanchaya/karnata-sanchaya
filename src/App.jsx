...
const cultureColors={architecture:'#7a4aa5','visual-art':'#c05c78',literature:'#355fa8',performance:'#dc7a28',music:'#16867a',celebration:'#d09222','religious-tradition':'#9a4f35',craft:'#537a42'}
...
function LocalizedMapLabels({locale,capitalIds,districtGeojson,showDistricts}){
  ...
  const placeLabels=atlasData.places.filter(place=>{
    ...
    return inKarnataka&&(capitalSet.has(place.id)||zoom>=7)
  })
  const districtLabels=showDistricts&&zoom>=7.5?(districtGeojson?.features||[]).map(feature=>({id:feature.properties.id,name:feature.properties.districtName?.kn,center:featureCenter(feature)})).filter(item=>item.name&&item.center):[]
...
function EvidenceChecks({verification,t}){
  ...
  return <div className="evidence-checks" aria-label={t.verificationChecks}><strong>{t.verificationChecks}</strong>{[['photoLicence',t.photoLicenceCheck],['protectionRegister',t.protectionRegisterCheck],['managingAuthority',t.managerCheck]].map(([key,label])=><span key={key} className={checks[key].status}><i>{checks[key].status==='verified'||checks[key].status==='matched'||checks[key].status==='identified'?'✓':'!'}</i>{label}: {t.checkStatusLabels[checks[key].status]||checks[key].status}</span>)}</div>
}

function DistrictAuditSection({locale,t}){
  ...
  const filtered=atlasData.heritageAudits.filter(audit=>{
    ...
    return haystack.includes(auditQuery.toLowerCase())&&(category==='all'||audit.categoryCoverage[category]!=='unassessed')
  })
...
function InscriptionAuditSection({locale,t,onChooseInscription}){
  ...
  return <section className="inscription-audit" id="inscription-audit">
    <div className="inscription-audit-head"><div><p className="eyebrow">{seeded}/31 {t.districtsSeeded} · {candidateDistricts} {t.candidateIdentified} · {candidateCount} {t.priorityCandidates}</p><h2>{t.inscriptionAuditTitle}</h2><p>{t.inscriptionAuditIntro}</p></div><label><span>{t.searchDistricts}</span><input value={auditQuery} onChange={event=>setAuditQuery(event.target.value)} placeholder={t.searchDistricts}/></label></div>
...
export default function App(){
  const publicViews=['atlas','relations','literature','epigraphy','districts','inscriptions','evidence','research','community']
  const initialHash=window.location.hash.slice(1)
  const [admin,setAdmin]=useState(()=>initialHash==='admin')
  ...
  const [layers,setLayers]=useState({boundaries:true,territorialReach:true,districts:true,heritageSites:true,culture:true,inscriptions:true,researchCandidates:true,events:true,modern:true})
  const [query,setQuery]=useState('')
  ...
  useEffect(()=>{let active=true;fetch(`${import.meta.env.VITE_COMMUNITY_API_URL||''}/api/auth/me`,{credentials:'include'}).then(response=>response.ok?response.json():null).then(data=>{if(active){setCommunityUser(data?.user||null);setCanExport(Boolean(data?.user?.canExport))}}).catch(()=>{if(active){setCommunityUser(null);setCanExport(false)}});return()=>{active=false}},[view])
  useEffect(()=>{const syncHash=()=>{const hash=window.location.hash.slice(1);setAdmin(hash==='admin');if(publicViews.includes(hash))setView(hash);else if(hash!=='admin')setView('atlas')};window.addEventListener('hashchange',syncHash);return()=>window.removeEventListener('hashchange',syncHash)},[])
  ...
  const heritageFilterPool=heritageCandidates.filter(site=>(selectedDistrict==='all'||site.auditId===selectedDistrict)&&(heritageCategory==='all'||site.category===heritageCategory))
  const visibleHeritage=heritageFilterPool.filter(site=>site.startYear!=null&&site.startYear<=year)
    ...
    {view==='atlas'&&<main id="atlas">
      ...
      <section className={`map-stage theme-${mapTheme} ${compareYear!=null?'compare-mode':''}`}>
        <MapContainer center={[14.7,76.2]} zoom={6.35} minZoom={3} scrollWheelZoom>
          ...
          <LocalizedMapLabels locale={locale} capitalIds={activeCapitalIds} districtGeojson={districtGeojson} showDistricts={layers.districts}/>
          ...
          {layers.inscriptions&&activeInscriptions.map(item=><CircleMarker key={item.id} center={item.coords} radius={selectedInscription?.id===item.id?9:5} pathOptions={{color:'#f4dfb1',fillColor:'#1d1813',fillOpacity:1,weight:selectedInscription?.id===item.id?3:2}} eventHandlers={{click:()=>chooseInscription(item)}}></CircleMarker>)}
          ...
          {layers.culture&&activeCulture.map(item=><CircleMarker key={item.id} center={item.coords} radius={selectedCulture?.id===item.id?10:7} pathOptions={{color:'#fff',fillColor:cultureColors[item.category],fillOpacity:1,weight:2}} eventHandlers={{click:()=>chooseCulture(item)}}><Popup><strong>{primary(item.name,locale)}</strong><br/>{t.cultureCategories[item.category]} · {item.date.from} {t.ce}<br/>{primary(item.description,locale)}</Popup></CircleMarker>)}
          ...
          {layers.heritageSites&&visibleHeritage.map(site=><CircleMarker key={`heritage-${site.id}`} center={site.coords} radius={site.verification.verificationStatus==='verified'?7:5} pathOptions={{color:'#fff',fillColor:heritageStatusColors[site.verification.verificationStatus],fillOpacity:.95,weight:2}}><Popup><strong>{primary(site.name,locale)}</strong><br/><span>{secondary(site.name,locale)}</span><br/>{primary(site.district,locale)} · {t.verificationLabels[site.verification.verificationStatus]}<br/>{site.verification.protectionStatus[0]&&<>{primary(site.verification.protectionStatus[0].designation,locale)}<br/></>}{site.creationPhase&&<>{t.timelineAppearance}: {evidenceDateLabel(site.creationPhase.date,locale)}<br/>{primary(site.creationPhase.name,locale)}<br/></>}{t.coordinates}: {site.coords[0].toFixed(5)}, {site.coords[1].toFixed(5)}{site.verification.siteCitations[0]&&<><br/><a href={site.verification.siteCitations[0].url} target="_blank" rel="noreferrer">{t.openSource} ↗</a></>}</Popup></CircleMarker>)}
      ...
      {selectedEvent?<EventDetail event={selectedEvent} locale={locale} t={t} onClose={()=>setSelectedEvent(null)}/>:selectedTerritory?<TerritoryDetail territory={selectedTerritory} locale={locale} t={t} onClose={()=>setSelectedTerritory(null)}/>:selectedCulture?<CultureDetail item={selectedCulture} locale={locale} t={t} onClose={()=>setSelectedCulture(null)}/>:selectedPerson?<PersonDetail person={selectedPerson} locale={locale} t={t} onClose={()=>setSelectedPerson(null)} onChooseRecord={choosePersonRecord}/>:selectedInscription?<InscriptionDetail item={selectedInscription} locale={locale} t={t} onClose={()=>setSelectedInscription(null)}/>:selectedWork?<WorkDetail work={selectedWork} locale={locale} t={t} onClose={()=>setSelectedWork(null)}/>:chosen?<KingdomDetail chosen={chosen} locale={locale} t={t} year={year} literatureWorks={literatureWorks} inscriptionRecords={inscriptionRecords} usesNearestLiterature={!activeLiterature.length} onChooseWork={chooseWork} onChooseInscription={chooseInscription} onChoosePerson={choosePerson}/>:<BceResearchContext locale={locale} t={t} year={year} onOpenEpigraphy={()=>navigateView('epigraphy')}/>}
    </main>}
    {view==='research'&&<Suspense fallback={<PortalFallback/>}><ResourcesCollaborations locale={locale} methodCards={t.cards} canExport={canExport}/></Suspense>}
    {view==='relations'&&<Suspense fallback={<PortalFallback/>}><GlobalRelationsExplorer locale={locale} mapTheme={mapTheme} canExport={canExport} setMapTheme={setMapTheme} onOpenAtlas={item=>{if(item.kind==='event'){const event=events.find(record=>record.id===item.id);if(event)chooseEvent(event)}else{const territory=territorialExtents.find(record=>record.id===item.id);if(territory)chooseTerritory(territory)}navigateView('atlas')}}/></Suspense>}
    {view==='inscriptions'&&<div className="portal-page" id="inscriptions"><InscriptionAuditSection locale={locale} t={t} onChooseInscription={item=>{chooseInscription(item);navigateView('atlas')}}/></div>}
    {view==='districts'&&<div className="portal-page" id="districts"><DistrictAuditSection locale={locale} t={t}/></div>}
    {view==='literature'&&<Suspense fallback={<PortalFallback/>}><LiteratureEpigraphyExplorer kind="literature" locale={locale} mapTheme={mapTheme} canExport={canExport} onOpenAtlas={item=>{chooseWork(item);navigateView('atlas')}}/></Suspense>}
    {view==='epigraphy'&&<Suspense fallback={<PortalFallback/>}><LiteratureEpigraphyExplorer kind="epigraphy" locale={locale} mapTheme={mapTheme} canExport={canExport} isCommunityMember={Boolean(communityUser)} onOpenAtlas={item=>{chooseInscription(inscriptionById.get(item.id)||item);navigateView('atlas')}}/></Suspense>}
    {view==='evidence'&&<Suspense fallback={<PortalFallback/>}><EvidenceWorkflow locale={locale} canExport={canExport}/></Suspense>}
    {view==='community'&&<Suspense fallback={<PortalFallback/>}><Community locale={locale}/></Suspense>}
