...
const timelineStories=[
  ...
  ...contextualExternalKingdoms.map(record=>({...record,year:record.date.from,storyKind:'external-kingdom',storyCategory:'externalKingdoms',storyLabel:record.capitalName,coords:[record.center[1],record.center[0]]})),
...
function EventRail({locale,t,year,stories,selectedId,onSelect,onCategoryChange}){
  ...
  const timelineCategories=['all','wars','freedom','political','connections','people','literature','periodicals','artifacts','inscriptions','temples','monuments','reigns','territory','externalKingdoms','culture']
  const chooseCategory=value=>{setCategory(value);onCategoryChange?.(value)}
  const needle=query.trim().toLowerCase()
  const visibleStories=compactTimelineStories(stories.filter(story=>(category==='all'||story.storyCategory===category)&&(!needle||JSON.stringify(story).toLowerCase().includes(needle))))
  const isStorySelected=story=>story.id===selectedId||story.timelineGroupIds?.includes(selectedId)
  const selectedIndex=visibleStories.findIndex(isStorySelected)
  const primaryStory=story=>story.timelineGroupItems?.find(item=>item.id===selectedId)||story.timelineGroupItems?.[0]||story
  const moveSelection=event=>{if(!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','Home','End'].includes(event.key)||!visibleStories.length)return;event.preventDefault();const current=selectedIndex<0?0:selectedIndex;const next=event.key==='Home'?0:event.key==='End'?visibleStories.length-1:Math.max(0,Math.min(visibleStories.length-1,current+(['ArrowLeft','ArrowUp'].includes(event.key)?-1:1)));onSelect(primaryStory(visibleStories[next]))}
  useEffect(()=>{const handler=event=>{if(!event.target.closest?.('.event-rail')||['INPUT','SELECT','TEXTAREA'].includes(event.target.tagName))return;moveSelection(event)};window.addEventListener('keydown',handler);return()=>window.removeEventListener('keydown',handler)},[selectedIndex,visibleStories,selectedId])
  const colorFor=story=>story.storyKind==='research-candidate'?'#b06a1f':story.storyKind==='event'?(story.reach?connectionColors[story.connectionCategory]:eventColors[story.type]||'#785f45'):story.storyKind==='literature'?cultureColors.literature:story.storyKind==='periodical'?'#5f6796':story.storyKind==='artifact'?'#8b5aa5':story.storyKind==='person'?'#b26a22':story.storyKind==='inscription'?'#9a6b24':story.storyKind==='reign'?'#315f91':story.storyKind==='territory'?territoryColors[story.classification]:story.storyKind==='external-kingdom'?story.color:story.storyCategory==='temples'?'#a65f24':cultureColors[story.category]||'#8b5aa5'
  ...
  return <section className="event-rail"><div className="event-rail-title"><strong>{t.timelineStories}</strong><label><span>{t.searchTimeline}</span><input aria-label={t.searchTimeline} value={query} onChange={event=>setQuery(event.target.value)} placeholder={t.searchTimelinePlaceholder}/></label></div><select className="timeline-category-select" aria-label={t.timelineCategoryLabel} value={category} onChange={event=>chooseCategory(event.target.value)}>{timelineCategories.map(value=><option key={value} value={value}>{t.timelineCategories[value]}</option>)}</select><div className="timeline-filters">{timelineCategories.map(value=><button key={value} className={category===value?'active':''} onClick={()=>chooseCategory(value)}><span aria-hidden="true">{value==='all'?'':objectIcon(value==='temples'?'temple':value==='monuments'?'monument':value==='wars'?'war':value==='freedom'?'person':value==='connections'?'connection':value==='inscriptions'?'inscription':value==='people'?'person':value==='reigns'?'reign':value==='territory'||value==='externalKingdoms'?'territory':value==='periodicals'?'literature':value==='artifacts'?'artifact':value)}</span>{t.timelineCategories[value]}</button>)}</div><div className="event-track">{visibleStories.map(story=>{const kind=objectKindFor(story);const storyDate=story.date||{};const periodText=(story.storyKind==='reign'||story.storyKind==='external-kingdom')&&storyDate.to!=null?`${timelineYearLabel(storyDate.from,locale)}–${timelineYearLabel(storyDate.to,locale)}`:`${storyDate.precision==='circa'?(locale==='kn'?'ಸು. ':'c. '):''}${timelineYearLabel(story.year,locale)}`;const groupCount=story.timelineGroupCount||1;return <button key={story.timelineCardId||`${story.storyKind}-${story.id}`} data-story-category={story.storyCategory} className={`${story.storyKind} ${story.reach?'connection-event':''} ${groupCount>1?'compact-group':''} ${isStorySelected(story)?'selected':''} ${Math.abs(story.year-year)<=18?'near':''}`} onClick={()=>onSelect(primaryStory(story))} style={{'--event-color':colorFor(story)}} title={groupCount>1?story.timelineGroupItems.map(item=>primary(item.name,locale)).join(' · '):undefined}><i aria-hidden="true"><span>{objectIcon(kind)}</span></i><span>{periodText}</span><strong>{primary(story.name,locale)}</strong><em>{labelFor(story)}{groupCount>1?` · +${groupCount-1}`:''}</em></button>})}{visibleStories.length===0&&<p className="timeline-empty">{t.noTimelineStories}</p>}</div></section>
}
...
export default function App(){
  ...
  const [layers,setLayers]=useState({boundaries:true,territorialReach:true,externalKingdoms:false,districts:true,heritageSites:true,culture:true,periodicals:true,artifacts:true,temples:true,inscriptions:true,researchCandidates:true,events:true,people:true,modern:true})
  const [query,setQuery]=useState('')
  ...
  const searchIndex=useMemo(()=>[
    ...
    ...contextualExternalKingdoms.map(record=>({kind:'polity',id:record.id,name:record.name,year:record.date.from,polityId:record.id,review:record.review?.status,record})),
  ...
  const activeCoreKingdoms=useMemo(()=>kingdoms.filter(k=>year>=k.start&&year<=k.end),[year])
  const activeExternalKingdoms=useMemo(()=>contextualExternalKingdoms.filter(k=>year>=k.start&&year<=k.end),[year])
  const active=useMemo(()=>layers.externalKingdoms?[...activeCoreKingdoms,...activeExternalKingdoms]:activeCoreKingdoms,[activeCoreKingdoms,activeExternalKingdoms,layers.externalKingdoms])
    ...
    {view==='atlas'&&<main id="atlas" className={mapOnlyMode?'atlas-map-only-main':''}>
      ...
      <section className={`map-stage theme-${mapTheme} ${compareYear!=null?'compare-mode':''}`}>
        ...
        <MapContainer center={[atlasMapView.lat,atlasMapView.lng]} zoom={atlasMapView.zoom} minZoom={3} scrollWheelZoom>
          ...
          {layers.externalKingdoms&&activeExternalKingdoms.map(polity=>{
            const selectedContext=highlightedKingdom?.id===polity.id
            return <Polygon key={`context-${polity.id}`} positions={polity.polygon} pathOptions={{className:'external-context-boundary',color:polity.color,fillColor:polity.color,fillOpacity:selectedContext ? .18 : .07,weight:selectedContext?3:1.5,dashArray:'6 7'}} eventHandlers={{click:()=>{setSelected(polity.id);setSelectedEvent(null);setSelectedTerritory(null);setSelectedCulture(null);clearRecordDetails();if(polity.contextScope==='world')setScope('world');else if(scope==='karnataka')setScope('india')}}}></Polygon>
          })}
        ...
        <EventRail locale={locale} t={t} year={year} stories={layers.externalKingdoms?timelineStories:timelineStories.filter(story=>story.storyCategory!=='externalKingdoms')} selectedId={selectedEvent?.id||selectedTerritory?.id||selectedPerson?.id||selectedWork?.id||selectedInscription?.id||selectedCulture?.id||selectedPeriodicalSite?.id||selectedSearchPlace?.reviewCandidateId||selectedSearchPlace?.periodicalSiteId||selectedSearchPlace?.artifactId||selected} onSelect={chooseTimelineStory} onCategoryChange={coordinateTimelineCategory}/>
