...
function AuditMap({kind,locale,t,districtGeojson,heritageSites=[],inscriptions=[],candidates=[]}){
  ...
  return <section className={`audit-map audit-map-${kind}`} aria-label={title}><div className="audit-map-heading"><div><p className="eyebrow">{locale==='kn'?'ನಕ್ಷೆ ವೀಕ್ಷಣೆ':'Map view'}</p><h3>{title}</h3><p>{note}</p></div><span>{markers.length} {locale==='kn'?'ಸ್ಥಳಗಳು':'locations'}</span></div><MapContainer center={[15.3,75.7]} zoom={6} minZoom={5} scrollWheelZoom={false}><AuditMapViewport points={markers.map(marker=>marker.coords)}/><TileLayer attribution="&copy; OpenStreetMap contributors" url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"/>{isDistricts&&districtGeojson&&<GeoJSON interactive={false} data={districtGeojson} style={{color:'#8790b5',weight:1,fillColor:'#e8ebff',fillOpacity:.12,dashArray:'4 5'}}/>}{markers.map(marker=><CircleMarker key={marker.id} center={marker.coords} radius={marker.kind==='candidate'?5:4} pathOptions={{color:marker.kind==='candidate'?'#b06a1f':'#3f37c9',fillColor:marker.kind==='candidate'?'#fff0c9':'#5361c9',fillOpacity:.9,weight:1.5}}><Popup><strong>{primary(marker.name,locale)}</strong><br/><small>{primary(marker.context,locale)}{marker.year?` · ${marker.year} ${locale==='kn'?'ಕ್ರಿ.ಶ.':'CE'}`:''}</small><br/><span>{marker.kind==='heritage'?t.verificationLabels[marker.status]||marker.status:marker.kind==='candidate'?(locale==='kn'?'ಸಂಶೋಧನಾ ಅಭ್ಯರ್ಥಿ':'Research candidate'):(locale==='kn'?'ನಕ್ಷೆಗೊಳಿಸಿದ ಶಾಸನ':'Mapped inscription')}</span></Popup></CircleMarker>)}</MapContainer><div className="audit-map-legend"><span><i className="audit-map-dot mapped"></i>{isDistricts?(locale==='kn'?'ಪರಂಪರೆ ಅಭ್ಯರ್ಥಿ':'Heritage candidate'):(locale==='kn'?'ನಕ್ಷೆಗೊಳಿಸಿದ ಶಾಸನ':'Mapped inscription')}</span>{!isDistricts&&<span><i className="audit-map-dot candidate"></i>{locale==='kn'?'ಪರಿಶೀಲನೆ ಬಾಕಿ ಅಭ್ಯರ್ಥಿ':'Pending-review candidate'}</span>}{isDistricts&&<span><i className="audit-map-boundary"></i>{locale==='kn'?'ಜಿಲ್ಲಾ ಗಡಿ':'District boundary'}</span>}</div></section>
}
