...
export default function App(){
  ...
  const [layers,setLayers]=useState({boundaries:true,territorialReach:true,districts:true,heritageSites:true,culture:true,temples:true,inscriptions:true,researchCandidates:true,events:true,people:true,modern:true})
  const [query,setQuery]=useState('')
  ...
  const coordinateTimelineCategory=category=>{const layerByCategory={wars:'events',political:'events',connections:'events',people:'people',inscriptions:'inscriptions',temples:'temples',monuments:'culture',culture:'culture',reigns:'boundaries',territory:'territorialReach'};const layer=layerByCategory[category];if(layer)enableMapLayer(layer)}
  ...
  return <div className="app-shell" lang={locale}>
    ...
    {view==='atlas'&&<main id="atlas">
      ...
      <section className={`map-stage theme-${mapTheme} ${compareYear!=null?'compare-mode':''}`}>
        ...
        <label className="people-map-toggle"><input type="checkbox" checked={layers.people} onChange={()=>setLayers(value=>({...value,people:!value.people}))}/><span>{t.people} · {activePeople.length}</span></label>
        ...
        <EventRail locale={locale} t={t} year={year} stories={timelineStories} selectedId={selectedEvent?.id||selectedTerritory?.id||selectedPerson?.id||selectedWork?.id||selectedInscription?.id||selectedCulture?.id||selectedSearchPlace?.reviewCandidateId} onSelect={chooseTimelineStory} onCategoryChange={coordinateTimelineCategory}/>
