...
try{
  const result=await transaction(async db=>{
    ...
    if(!merged.changed)return {changed:false,revision:latest.revision,added:0,updated:0}
    ...
    return {changed:true,revision:saved.revision,added:merged.added,updated:merged.updated}
  })
  console.log(result.changed?`Created MariaDB dataset revision ${result.revision}; added ${result.added} and enriched ${result.updated} repository seed records.`:`MariaDB dataset revision ${result.revision} already contains the repository seed.`)
}finally{
  await closeDatabase()
}
