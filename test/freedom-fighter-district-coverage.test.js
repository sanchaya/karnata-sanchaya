...
test('exact Bengaluru identities and martyrs retain authority level evidence without premature promotion', () => {
  const dictionarySource = atlasData.sources.find(record => record.id === 'src-india-culture-dictionary-martyrs-v5')
  const kcReddy = fighters.find(record => record.id === 'person-bengaluru-ff-kc-reddy')
  const chinnakannu = fighters.find(record => record.id === 'person-bengaluru-ff-chinnakannu')
  const appayya = fighters.find(record => record.id === 'person-bengaluru-ff-appayya')
  const annayyappa = fighters.find(record => record.id === 'person-bengaluru-ff-annapayya')
  const dastagir = fighters.find(record => record.id === 'person-bengaluru-ff-dastagir-sab')
  const tippayya = fighters.find(record => record.id === 'person-bengaluru-ff-tippayya-1942')
  const provisionalMartyrs = ['thippaiah-1942','ponnaswami','subhasing-ramsing','thimmanna-das','gundappa-1937','prahalada-setty']
    .map(slug => fighters.find(record => record.id === `person-bengaluru-ff-${slug}`))

  assert.equal(dictionarySource?.type, 'government-reference-book')
  assert.equal(dictionarySource?.year, 2018)
  assert.equal(dictionarySource?.isbn, '978-81-938176-1-2')
  assert.ok(dictionarySource?.alternateUrls?.some(url => url.includes('archive.org/details/')))
  assert.equal(kcReddy?.name.en, 'Kyasamballi Chengalaraya Reddy (K. C. Reddy)')
  assert.deepEqual(kcReddy?.date, { from:1902, to:1976, era:'CE', precision:'range' })
  assert.ok(kcReddy?.districtAssociations?.some(item => item.districtId === 'audit-kolar' && item.kind === 'birthplace'))
  assert.ok(chinnakannu?.citations?.some(item => item.sourceId === dictionarySource.id))
  assert.ok(chinnakannu?.districtAssociations?.some(item => item.kind === 'martyrdom'))
  assert.equal(appayya?.martyrdomDate, '1942-08-17')
  assert.equal(annayyappa?.martyrdomDate, '1937-10-25')
  assert.equal(dastagir?.martyrdomDate, '1921-11-18')
  assert.equal(tippayya?.ageAtDeath, 15)
  assert.ok(tippayya?.citations?.some(item => item.sourceId === dictionarySource.id && item.locator.includes('printed p. 238')))
  assert.ok(provisionalMartyrs.every(record => record?.review.status === 'needs-review'))
})
