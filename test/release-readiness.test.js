...
test('source-volume extraction sprint is assembled into the atlas dataset', () => {
  ...
  for (const collection of ['sourceVolumeExtractionSources','sourceVolumeExtractionPlaces','sourceVolumeExtractionPeople','sourceVolumeExtractionEvents','sourceVolumeExtractionInscriptions','sourceVolumeExtractionInscriptionEditions','sourceVolumeExtractionManuscriptWitnesses','sourceVolumeExtractionCoinRecords','sourceVolumeExtractionHeritage']) {
...
test('coin explorer is public, searchable and evidence-gated', () => {
  ...
  assert.match(coinSource, /const gateRequiredFields=/, 'coin evidence gates must show specimen-level required fields')
  assert.match(coinSource, /coin-publication-state/, 'coin details must label catalogue-grade versus review-only records')
  assert.match(coinSource, /coin-next-action/, 'coin details must tell reviewers the next numismatic action')
  ...
  assert.match(coinStylesSource, /\.coin-publication-state/, 'coin publication state needs dedicated styling')
  assert.match(coinStylesSource, /\.coin-next-action/, 'coin next action needs dedicated styling')
})
...
test('tablet layouts preserve maps, readable cards and non-obstructing controls', () => {
  ...
  assert.match(coinStylesSource, /\.coin-page\{display:block!important;height:auto!important/, 'coin explorer must not inherit the atlas three-column main shell')
...
test('the Epigraphy Explorer exposes the P2 corpus review queue', () => {
  ...
  assert.match(explorerSource, /edition-locator-review/, 'edition packets must expose locator-review next actions')
  assert.match(explorerSource, /scriptPhaseById/, 'edition packet details must show linked script-evolution phases')
  ...
  assert.match(explorerSource, /ItihasaDarshanaCorpus/, 'the epigraphy page must expose linked research-volume review leads')
  assert.match(explorerSource, /karnatakaParampareSources/, 'the epigraphy page must include Karnataka Parampare source leads')
  assert.match(explorerSource, /atlasLinks/, 'research volumes must surface atlas link groups')
  assert.match(explorerSource, /itihasa-locator-hint/, 'Karnataka Parampare locator hints must be visible in the research-volume drawer')
...
test('the Evidence Workflow exposes P2 material evidence task streams', () => {
  ...
  for (const domain of ['coinage','manuscripts','genealogy','boundaries','scripts','sourceLinks']) {
  ...
  for (const field of ['catalogue','repositoryRecord','relationshipProof','sampleWitnesses','archiveLocator','articlePageReview']) {
    assert.match(evidenceSource, new RegExp(`${field}:\\{submissionType`), `${field} needs a task template`)
  }
  ...
  assert.match(evidenceSource, /sourceReviewVolumes=atlasData\.sources\.filter/, 'research-volume atlas links must feed the evidence workflow')
  assert.match(evidenceSource, /p2-karnataka-parampare-article-review/, 'Karnataka Parampare volumes must create source-link review tasks')
  assert.match(evidenceSource, /Locator hints:/, 'source-link tasks must include page and OCR locator hints when available')
...
test('Evidence Workflow exposes P3 reviewer notes and status history', () => {
  ...
  assert.match(evidenceSource, /const reviewerDashboard=\[\.\.\.tasks\.reduce/, 'reviewer operations must summarize assignments across the board')
  assert.match(evidenceSource, /const readyForPromotionQueue=p1PromotionCandidates\.map/, 'reviewed P1 candidates need a ready-for-promotion queue')
  assert.match(evidenceSource, /className="reviewer-ops-panel"/, 'reviewer operations must be visible before the board')
  assert.match(evidenceSource, /focusPromotionPacket\(item\.candidate\)/, 'promotion queue cards must narrow the task board')
  ...
  assert.match(evidenceStylesSource, /\.reviewer-ops-panel/, 'reviewer operations need dedicated compact workflow styles')
...
test('relations explorer exposes genealogy and feudatory network edges', () => {
  ...
  assert.match(relationsSource, /relations-network-mini/, 'the network panel must include a graph visualization')
  assert.match(relationsSource, /positions\[node\.id\]/, 'network nodes must be positioned visually instead of only listed')
  assert.match(relationsStylesSource, /\.relations-network-panel/, 'the network panel needs dedicated layout styles')
  assert.match(relationsStylesSource, /\.relations-network-mini/, 'the network graph needs dedicated visual styles')
})
...
test('relations layout keeps the map and timeline usable on narrow screens', () => {
  ...
  assert.match(relationsSource,/const \[open,setOpen\]=useState\(false\)/, 'the review matrix should collapse by default')
